...
def _apply_tree_style():
    ...
    style.map("Plants.Treeview.Heading",
...
class PlantsView(ctk.CTkFrame):
    ...
    _COL_WIDTHS = [40, 175, 140, 70, 100, 150, 100]
    ...
    def _build(self):
        ...
        search_row.grid(row=1, column=0, sticky="ew", padx=20, pady=(0, 6))
        ...
        ctk.CTkEntry(
            ...
            placeholder_text="Search species, nickname, location...",
            height=32,
    ...
    def _render(self):
        ...
        if active:
            self._status_bar.configure(
                text=f"Showing {shown} of {total} plant{'s' if total != 1 else ''}",
                text_color=("#b87c1a", "#d49a20"),
            )
        else:
            self._status_bar.configure(
                text=f"{total} mother plant{'s' if total != 1 else ''}",
                text_color=("gray50", "gray60"),
            )
    ...
    def _edit(self):
        ...
        if id_ is None:
            return
        dlg = PlantDialog(self, plant_row=PlantsModel.get_by_id(id_))
        self.wait_window(dlg)
        if dlg.result is not None:
            self.refresh()
